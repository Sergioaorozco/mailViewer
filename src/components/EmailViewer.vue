...
const isHtmlComplete = (html) => {
  ...
  // Basic heuristic: must contain a closing </html> or </body>
  ...
  <section class="p-5 flex gap-x-3 text-xl justify-between items-center">
    <div class="flex gap-x-2 items-center">
      ...
      <span class="leading-none">
        <h1>Email Viewer</h1>
        <h2 class="text-zinc-600 font-light text-lg">{{ uploadedFilename }}</h2>
      </span>
    </div>
      ...
      <article class="w-2/3 border border-zinc-300 rounded-lg overflow-clip">
        ...
        <template v-if="selectedEmail.subject">
          ...
          <div v-else class="w-full h-full p-6 overflow-auto text-zinc-700">
            <div v-if="selectedEmail.text && selectedEmail.text.length" class="whitespace-pre-wrap">
              <!-- Show plain text body while HTML is still being built -->
              {{ selectedEmail.text }}
            </div>
            <div v-else class="flex flex-col justify-center items-center w-full h-full text-zinc-400">
              <Mail class="size-14"></Mail>
              <p class="text-zinc-600">Cargando contenido del email...</p>
            </div>
          </div>
        </template>
        <div v-else class="flex flex-col justify-center items-center w-full h-full text-zinc-400">
