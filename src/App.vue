  ...
  <main class="flex px-12 gap-x-2 my-5" v-if="renderedEmail[0]">
    <section class="w-1/3 px-2">
      ...
      <div class="flex flex-col">
        <div @click="selectEmail(email, index)" :class="{'bg-emerald-700 text-white hover:bg-emerald-800': selectedEmail === renderedEmail[index]}" class="flex gap-x-4 py-4 px-2 items-start bg-neutral-50 text-sm border border-neutral-200 hover:bg-neutral-200 cursor-pointer" v-for="(email, index) in renderedEmail" :key="email.date">
